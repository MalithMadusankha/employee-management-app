...
import { UpdateEmployeeHandler } from "../../controller/EmployeeHandler";

export default function UpdateEmployeeForm({ item, toggle }) {
  const [full_name, setFull_name] = useState(item?.full_name || "");
  const [name_with_initials, setName_with_initials] = useState(
    item?.name_with_initials || ""
  );
  const [display_name, setDisplay_name] = useState(item?.display_name || "");
  const [gender, setGender] = useState(item?.gender || "");
  const [date_of_birth, setDate_of_birth] = useState(item?.date_of_birth || "");
  const [mobile_number, setMobile_number] = useState(item?.mobile_number || "");
  const [designation, setDesignation] = useState(item?.designation || "");
  const [employee_type, setEmployee_type] = useState(item?.employee_type || "");
  const [email, setEmail] = useState(item?.email || "");
  const [join_date, setJoin_date] = useState(item?.join_date || "");
  const [personal_notes, setPersonal_notes] = useState(
    item?.personal_notes || ""
  );
  const [experience, setExperience] = useState(item?.experience || null);
  const [salary, setSalary] = useState(item?.salary || null);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await UpdateEmployeeHandler(employeeObj, item._id);
      setIsLoading(false);
      setIsSuccess(true);
    } catch (error) {
      setIsLoading(false);
    ...
    <>
      <Form onSubmit={handleSubmit}>
        ...
        {isLoading ? (
          ...
          <Alert color="success"> Employee Updated</Alert>
        ) : (
